from q_learning import save_history, QLearningAgent
...
class SarsaAgent(QLearningAgent):
    def __init__(self, n_states, n_actions, learning_rate=0.1, gamma=0.99, epsilon=1.0, epsilon_decay=0.995, epsilon_min=0.01, q_table_filename=None):
        super().__init__(n_states, n_actions, learning_rate, gamma, epsilon, epsilon_decay, epsilon_min, q_table_filename)
    
    def learn(self, state, action, reward, next_state, next_action, done):
        old_value = self.q_table[state, action]
        next_value = self.q_table[next_state, next_action]
        
        new_value = (1 - self.lr) * old_value + self.lr * (reward + self.gamma * next_value * (1 - done))
    ...
    for episode in range(n_episodes):
        ...
        if (episode + 1) % episodes == 0:
            ...
            if solve_rate >= 90 and scramble != 12:
                q_table_filename = f'sarsa_table_scramble_{scramble}_solve_rate_{solve_rate:.2f}_avg_steps_{solve_steps:.2f}.npy'
                agent.save_q_table(q_table_filename)
                print(f"Saved SARSA table to {q_table_filename}")
            ...
            save_history(avg_reward, avg_steps, solve_rate, solve_steps, 'sarsa_history.json')
...
if __name__ == "__main__":
    ...
    MAX_STEPS = 50
    RENDER = False
    Q_TABLE_FILENAME = '1000w_sarsa.npy'
